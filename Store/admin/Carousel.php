...
if (isset($_POST['action'])) {
    if ($_POST['action'] == "addpic") {
        ...
        $conn->query("INSERT INTO carousel (carousel_pic_sort, carousel_pic_path)
        VALUE('" . trim($_POST['sorting']) . "', '" . $newname . "')");
        // echo "<script>alert('สำเร็จ!');window.location='?id=" . $_GET['id'] . "'</script>";
    }
    if ($_POST['action'] == "deletedata") {
        $chk = $conn->query("DELETE FROM carousel WHERE carousel_id = '" . $_POST['id'] . "'");
        if ($chk) {
    ...
    <div class="table-content mt-4">
        <div class="card">
            <div class="card-head pr-2">
                <div class="row">
                    <div class="col-sm-6">
                        <p>( เพิ่ม / ลบ ) รูปสินค้า => แบนเนอร์ </p>
                    </div>
                ...
                <div class="card-body">
                    <div class="box-tale">
                        <table class="table" id="dataal">
                            ...
                            <tbody>
                                <?php
                                $sql = $conn->prepare("SELECT * FROM carousel ORDER BY carousel_pic_sort ASC");
                                $sql->execute();
                                ...
                                ?>
                                        <tr>
                                            <td scope="row"><?php echo $res->carousel_pic_sort; ?></td>
                                            <td><img src="../include/img/<?php echo $res->carousel_pic_path; ?>" width="50%"></td>
                                            ...
                                            <td>
                                                <div class="dropdown open">
                                                    ...
                                                    <div class="dropdown-menu" aria-labelledby="triggerId">
                                                        <button class="dropdown-item" onclick="deletepic('<?php echo $res->carousel_id; ?>')">ลบ</button>
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
